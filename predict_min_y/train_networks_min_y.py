from .dataset_train_min_y import CrackDatasetTrain
from .dataset_test_min_y import CrackDatasetTest
import torch
...
def evaluate_performance(net,path):
    net.eval()
    ...
    print(mean_values)
    np.savetxt(path,mean_values)

def train_networks(network,name,lr, gamma, epochs = 100, batchsize = 256, optimizer = "ADAM",weight_decay = 0):
    ...
    for e in range(epochs):
        ...
        if min_valid_loss > valid_loss:
            ...
            if valid_loss < 30.0:
                evaluate_performance(net,name + ".txt")
